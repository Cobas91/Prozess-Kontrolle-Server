...
cronJob.scheduleJob(
  `${config.Daily_Job_Uhrzeit.sekunde} ${config.Daily_Job_Uhrzeit.minute} ${config.Daily_Job_Uhrzeit.stunde} * * *`,
  async function () {
    console.log(
      `Aufgabe KHK Import um ${config.Daily_Job_Uhrzeit.stunde}:${config.Daily_Job_Uhrzeit.minute} Uhr gestartet.`
    );
...
cronJob.scheduleJob(
  `${config.teams.dailyFeedback.time.sekunde} ${config.teams.dailyFeedback.time.minute} ${config.teams.dailyFeedback.time.stunde} * * *`,
  async function () {
    console.log(
      `Status Update an Teams gesendet ${config.teams.dailyFeedback.time.stunde}:${config.teams.dailyFeedback.time.minute} Uhr.`
    );
...
async function dailyFeedback(channel) {
  ...
  for (var kundenIndex in kunden) {
    ...
    for (var statusIndex in status) {
      ...
      var datenbank = await db.select("systeme", {
        Kunde: kundenName,
        Status: statusName,
      });
    ...
    sections: [
      {
        ...
        activitySubtitle:
          "<li>Versand Ready - Geräte liegen zum Kommisionieren und Versenden bereit.</li><br><li>Ready4Install - Geräte stehen aktuell in der Straße</li><br>",
        activityImage:
...
async function dealExcelDataLager(pfad) {
  ...
  var body = Data;
  console.log(
    "Begin Loop over all incomming Lager Data " + new Date().toLocaleString()
  );
  // 0 = Artikelnummer
  // 1 = Matchcode
  // 2 = Lager
  ...
  for (var index in body) {
    if (index > 0) {
      ...
      if (result.statusCode != 400) {
        ...
        log.add(
          `Job: Daily KHK Lager Import ${body[index][3]} already exists, updated Asset with new Data`
        );
      }
    }
  }
  log.add(
    `Job: Daily KHK Lager Import ended......${new Date().toLocaleString()}`
  );
  console.log(
    "Stop Loop over all incomming Lager Data " + new Date().toLocaleString()
  );
  return true;
}
async function dealExcelDataLieferscheine(pfad) {
  console.log(
    "Begin Loop over all incomming Lieferscheine Data " +
      new Date().toLocaleString()
  );
  var header = [
    "Artikelnummer",
  ...
  fs.createReadStream(pfad)
    ...
    .on("data", async (row) => {
      if (row.Belegart === "Lieferschein") {
        ...
        var systemToInsert = await db.select("systeme", {
          SN: row.Seriennummer,
        });
        if (systemToInsert.length < 1) {
          ...
          var updateResult = await db
            .update("systeme", toInsert, { SN: row.Seriennummer })
            .catch(function (err) {
              log.add(`Job: Daily KHK Lieferscheine Error ${err}`);
            });
          log.add(
            `Job: Daily KHK Lieferscheine Update Asset ${row.Seriennummer}`
          );
        }
      }
    })
    .on("end", async () => {
      console.log(
        "Stop Loop over all incomming Lieferscheine Data " +
          new Date().toLocaleString()
      );
    });
}
...
async function khkImportLieferscheine(pfad) {
  try {
    if (fs.existsSync(pfad)) {
      ...
      //in DB Logs schreiben
      log.add(
        `Job: Daily KHK Lieferscheine Import failed: File did not exist!`
      );
      return false;
    }
  } catch (err) {
